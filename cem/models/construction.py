...
from cem.models.template_model import inception_v3
...
def construct_model(
    ...
    if isinstance(config["c_extractor_arch"], str):
        ...
        elif config["c_extractor_arch"] == "inception_v3":
            c_extractor_arch = inception_v3
        else:
...
def load_trained_model(
    ...
    for k1 in list(state_dict.keys()):
        if k1 in model_state_dict:
            if state_dict[k1].shape != model_state_dict[k1].shape:
                print(f"Skipping {k1} due to shape mismatch")
                del state_dict[k1]
