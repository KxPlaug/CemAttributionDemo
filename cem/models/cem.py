...
class ConceptEmbeddingModel(ConceptBottleneckModel):
    ...
    def forward_for_c(
        ...
        c_pred = c_pred.view((-1, self.emb_size * 1))
        return c_pred
    
    def get_c_pred(self,c_sem,contexts,leave_concepts=None):
        c_pred = (
            contexts[:, :, :self.emb_size] * torch.unsqueeze(c_sem, dim=-1) +
            contexts[:, :, self.emb_size:] * (1 - torch.unsqueeze(c_sem, dim=-1))
        )
        if leave_concepts is not None:
            c_pred = c_pred.view((-1, self.emb_size * leave_concepts))
        else:
            c_pred = c_pred.view((-1, self.emb_size * self.n_concepts))
        return c_pred
